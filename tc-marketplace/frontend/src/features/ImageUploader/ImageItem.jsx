...
export default function ImageItem({ image, onDelete }) {
    ...
    return (
        <div className={`relative w-full h-32 overflow-hidden rounded border-3 ${image.uploadStatus === 'uploading' ? 'border-yellow-500 animate-pulse' : image.uploadStatus === 'uploaded' ? 'border-green-500' : 'border-gray-300' ? image.uploadStatus === 'error' : 'border-red-600' } cursor-pointer`}>
